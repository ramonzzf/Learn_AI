    ...
    for box1 in values.keys():
        for box2 in peers[box1]:
            if values[box1] == values[box2] and len(values[box1]) == 2:
                ## find the units where both box1 and box2 are in
                shared_units = [unit for unit in unitlist if box1 in unit and box2 in unit]
...
def eliminate(values):
    ...
    for item in values:
        if len(values[item]) == 1:
            s_boxes.append(item)
    for sitem in s_boxes:
        snum = values[sitem]
        for speer in peers[sitem]:
            if snum in values[speer]:
                values[speer] = values[speer].replace(snum,"")
    return values
...
def only_choice(values):
    ...
    for unit in unitlist:
        for snum in "123456789":
            count = 0
            sname = ""
            for box in unit:
                if snum in values[box]:
                    name = box
                    count += 1
            if count == 1:
                values[name] = snum
...
def search(values):
    ...
    minlen = -1
    for box in boxes:
        if len(values[box]) < minlen or minlen == -1:
            minlen = len(values[box])
            boxname = box
    if minlen == -1:
        return values
    # Now use recursion to solve each one of the resulting sudokus, and if one returns a value (not False), return that answer!
    for sdigit in values[boxname]:
        new_values = values.copy()
        new_values[boxname] = sdigit
